		...
		if len(dataLine) > 3 {
			if dataLine[1] == "::=" && dataLine[2] == "SEQUENCE" {
				...
				for {
					...
					str += "\t" + GetStrOutOfDash(dataLine5[0]) + "ValuePresent" + GetStrOutOfDash2(strings.Title(dataLineX[2])) + "" +
						" int = " + strconv.Itoa(k) + "\n"
					j++
					k++
				}
...
func CHOICE(datas []string, in int) (str string, out int) {
	...
	for {
		...
		str += "\t" + GetStrOutOfDash(datax[0]) + "Present" + GetStrOutOfDash(dataxx[0]) + " int = " + strconv.Itoa(x) + "\n"
		x++
	}
	str += ")\n"
	str += "\ntype " + GetStrOutOfDash(datax[0]) + " struct {\n"
...
func NGAPPROTOCOLEXTENSION(datas []string, in int) (str string, out int) {
	...
	if len(datax) == 4 {
		...
		for {
			...
			if dataxx[0] == "}" {
				if x == 1 {
					str += "\t" + GetStrOutOfDash(datax[0]) + "ExtensionValuePresentNothing int = 0\n"
				}
				break
				...
				str += "\t" + GetStrOutOfDash(datax[0]) + "ExtensionValuePresent" + GetStrOutOfDash2(dataxx[2]) + " int = " + strconv.Itoa(x) + "\n"
				x++
...
func NGAPPROTOCOLIES(datas []string, in int) (str string, out int) {
	...
	if len(datax) == 4 {
		...
		for {
			...
			if dataxx[0] == "}" {
				if x == 1 {
					str += "\t" + GetStrOutOfDash(strings.Title(datax[0])) + "TypeValuePresentNothing int = 0\n"
				}
				break
			} else if dataxx[0] == "..." {
				x++
				...
				str += "\t" + GetStrOutOfDash(datax[0]) + "TypeValuePresent" + GetStrOutOfDash2(dataxx[2]) + " int = " + strconv.Itoa(x) + "\n"
				x++
			} else {
				fmt.Println(in+1+x, "-", dataxx)
				x++
